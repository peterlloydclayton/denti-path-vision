/**
 * Test Application Submission Script (Node.js)
 * 
 * Run this script locally to generate and submit test applications.
 * Usage: node test/testApplicationSubmission.js
 * 
 * This script:
 * - Generates complete application data with all fields
 * - Creates unique emails: peter+[MM]-[DD]-[aaa]@vypple.com
 * - Submits to v2 endpoint using ngrok URL for CORS
 * - Tracks submissions in CSV file
 */
...
async function getNgrokUrl() {
  ...
  // Fallback to known ngrok URLs
  return 'https://f208ec16b693.ngrok-free.app';
}

// CSV file path
const CSV_FILE_PATH = path.join(__dirname, 'test-records.csv');
...
function generateSequentialCode(sequence) {
  const base = 26;
  let code = '';
  let num = sequence;
...
function generateEmail() {
  ...
  const sequence = getNextSequenceForToday();
...
// Generate document hash (Node.js version)
async function generateDocumentHash(text) {
  return crypto.createHash('sha256').update(text).digest('hex');
}

// Create signed PDF (Node.js version)
async function createSignedPDF(documentContent, signatureName, signatureDate) {
...
// Write to CSV file
function writeToCSV(name, email, success) {
  const now = new Date();
  ...
  try {
    ...
    if (!fs.existsSync(CSV_FILE_PATH)) {
      fs.writeFileSync(CSV_FILE_PATH, 'Name,Email,Submission Date/Time,Success Status\n');
    }
...
async function generateApplicationData() {
  ...
  const ipAddress = '127.0.0.1'; // Local IP for script
  const userAgent = 'Test-Script/1.0';
...
async function testSubmitApplication() {
  try {
    ...
    // Get ngrok URL for origin header
    const ngrokUrl = await getNgrokUrl();
    console.log('🌐 Using ngrok URL for origin:', ngrokUrl);
    ...
    // Submit to v2 endpoint with ngrok URL as origin
    // Note: In Node.js, we need to set the Origin header manually for CORS
    ...
    const response = await fetch('https://epkypzawqtpokmatjuzo.supabase.co/functions/v1/submit-patient-application_v2', {
      ...
      headers: {
        ...
        'Authorization': `Bearer ${supabaseAnonKey}`, // Also include as Bearer token
        'Origin': ngrokUrl, // Set origin to ngrok URL for CORS
        'Referer': ngrokUrl, // Also set referer
        'User-Agent': 'Test-Script/1.0 (Node.js)' // Required by v2 function validation
      },
...
if (isMainModule || process.argv[1]?.includes('testApplicationSubmission.js')) {
